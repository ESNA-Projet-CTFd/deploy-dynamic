        ...
        result.data.forEach(item => {
            ...
            if ($connInfo.length > 0) {
                ...
                if (!connHtml.includes('<a')) {
                    connHtml = connHtml.replace(/(https?:\/\/[^\s<"']+)/, url =>
            ...
            var x = setInterval(function() {
                ...
                if (distance < 0) {
                    ...
                    CTFd.lib.$('#' + instanceId + '_countdown').html(minutes + ':' + seconds);
                }
            }, 1000);
        });
...
function destroy_container(container) {
    ...
    CTFd.fetch("/api/v1/container?name=" + encodeURIComponent(container) +
        ...
        updateWarningModal({
            title: "Error",
            warningText: error.message || "An unknown error occurred while destroying the container.",
            buttonText: "Close",
            onClose: function() { get_docker_status(container); }
        });
    });
